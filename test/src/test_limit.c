...
void run_test_rlimit()
{
    ...
    rpl.rlim_cur = TINY_RANGE_ALLOCATION_SIZE;
    rpl.rlim_max = TINY_RANGE_ALLOCATION_SIZE;

    if (setrlimit(RLIMIT_DATA, &rpl) < 0)
        printf("setrlimit did not work\n");

    if (!(t = (char *)malloc(TINY_RANGE_ALLOCATION_SIZE - sizeof(t_block) - sizeof(t_range))))
        printf("malloc() should return ptr\n");
    free(t);

    if ((t = (char *)malloc(TINY_RANGE_ALLOCATION_SIZE)))
